...
- `POST_JUDGE_RETRIES = 3` — дополнительные циклы добора после судьи
- `SMALL_CLASS_TEMPERATURE = 0.95` — повышенная температура для классов с малым числом источников
- `RUT5_MIN_TEXT_LENGTH = 250`, `RUT5_MIN_LENGTH_RATIO = 0.35` — фильтр длины для paired-парафраза
...
| `augment_class(class_name, existing_texts, n_needed, paraphraser, n_original=None, paraphrase_sources=None, generation_attempt_offset=0)` | Набирает валидированный пул ruT5-парафразов. Прогрессивный порог сходства: 0.95 → 0.98. Температура повышается для малых классов и растёт после заданной попытки. Prompt-leak фильтр отключён, длина проверяется относительно источника. |
| `run(config_path, pipeline_cfg=None)` | Точка входа. Выполняет несколько циклов `ruT5 → validation → unload → vLLM judge → checkpoint`, пока классы не добраны или не исчерпаны post-judge retries. На финальном цикле может добрать лучшими валидированными кандидатами ниже порога судьи. |
