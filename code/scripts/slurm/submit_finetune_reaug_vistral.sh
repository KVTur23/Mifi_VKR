...
mkdir -p /mnt/pool/6/kvturanosov/VKR/finetune_runs/reaug_vistral_24b_qlora_cw_focal_g2/logs
...
FOCAL_RUN=$(sbatch --parsable $SBATCH_ARGS --dependency=afterok:$PRE scripts/slurm/finetune_reaug_vistral_cw_focal_g2_run.sbatch)
echo "focal run:     $FOCAL_RUN"
FOCAL_POST=$(sbatch --parsable $SBATCH_ARGS --dependency=afterany:$FOCAL_RUN scripts/slurm/finetune_reaug_vistral_cw_focal_g2_post.sbatch)
