...
mkdir -p /mnt/pool/6/kvturanosov/VKR/finetune_runs/reaug_qwen3_32b_qlora_no_cw/logs
...
NO_CW_RUN=$(sbatch --parsable --dependency=afterok:$PRE scripts/slurm/finetune_reaug_no_cw_run.sbatch)
echo "no cw run:     $NO_CW_RUN"
NO_CW_POST=$(sbatch --parsable --dependency=afterany:$NO_CW_RUN scripts/slurm/finetune_reaug_no_cw_post.sbatch)
echo "no cw post:    $NO_CW_POST"
