...
Запуск:
    python src/augmentation/stage3_paraphrase_v2.py --config config_models/aug_configs/model_vllm_32b.json
"""
...
def build_paraphrase_prompt(
    ...
    """Собирает промпт для deep-перефразирования одного текста."""
...
def augment_class(
    ...
    """
    Генерирует deep-парафразы для одного класса батчами.

    Схема: берём источники по кругу → генерируем с запасом →
...
def run(config_path: str | Path = DEFAULT_CONFIG, pipeline_cfg=None) -> None:
    ...
    for class_idx, (class_name, current_count) in enumerate(classes_to_augment.items()):
        ...
        print(f"\n[Этап 3 V2] Класс «{class_name}»: есть {current_count} "
        ...
        try:
            ...
            print(f"[Этап 3 V2] Ошибка при обработке класса «{class_name}»: {e}")
            print("[Этап 3 V2] Пропускаю класс, продолжаю с остальными")
            continue

        for text in generated:
            new_rows.append({TEXT_COL: text, LABEL_COL: class_name})

        print(f"[Этап 3 V2] Класс «{class_name}»: добавлено {len(generated)} парафразов")

        if new_rows:
            df_tmp = pd.concat([df, pd.DataFrame(new_rows)], ignore_index=True)
            save_checkpoint(df_tmp, stage=STAGE)
            print(f"[Этап 3 V2] Промежуточное сохранение: "
                  f"{class_idx + 1} классов обработано, {len(df_tmp)} записей")

    if new_rows:
        new_df = pd.DataFrame(new_rows)
        df = pd.concat([df, new_df], ignore_index=True)
        print(f"\n[Этап 3 V2] Всего добавлено {len(new_rows)} текстов")
    else:
        print("\n[Этап 3 V2] Новых текстов не сгенерировано")

    save_checkpoint(df, stage=STAGE)

    print("\n[Этап 3 V2] Итоговое распределение:")
    dist = get_class_distribution(df)
    for name, count in dist.items():
        marker = " ✓" if count >= TARGET_COUNT else " ✗"
        print(f"  «{name}»: {count}{marker}")

    print(f"\n[Этап 3 V2] Завершён. Всего записей: {len(df)}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Этап 3 v2: deep paraphrase для классов с < 50 примерами"
    )
    parser.add_argument(
        "--config",
        type=str,
