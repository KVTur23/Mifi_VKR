...
def build_prompt(template: str, class_name: str, examples: list[str]) -> str:
    """
    Собирает промпт для генерации одного письма.

    Показывает модели несколько случайных примеров из класса как контекст,
    ...
    Аргументы:
        ...
        examples:    список существующих текстов класса для контекста
...
def augment_class(
    ...
    while len(all_valid_texts) < n_needed and total_attempts < max_total_attempts:
        ...
        valid = validate_generated_texts([candidate], current_existing, class_name)
