"""
run_adalora.py — Запуск файнтюна Qwen3-32B + AdaLoRA (4bit база, SEQ_CLS)

Тонкая обёртка над SeqClsRunner. Вся специфика метода — в JSON-конфиге.
total_step для AdaLoRA вычисляется динамически в trainer_base.prepare().
"""
...
CONFIG_PATH = str(
    PROJECT_ROOT / "config_models" / "finetune_configs" / "qwen3_32b_adalora.json"
)
